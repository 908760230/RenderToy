...
void Texture::updateDescriptor()
{
	...
	descriptor.imageView = image->imageView();
...
void Texture::fromGLTFImage(tinygltf::Image& gltfImage, const std::string& path, VulkanDevice* device, VkQueue copyQueue)
{
	this->device = device;
	this->image = new VulkanImage(device);
	std::shared_ptr<VulkanBuffer> stagingBuffer;
	bool flag = isKtx(gltfImage.uri);
	if (!flag) {
		// use STB_Image
		...
		stagingBuffer = std::make_shared<VulkanBuffer>(*device, bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		...
		VulkanBuffer stagingBuffer(*device, textureSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		stagingBuffer.mapMemory();
	...
	this->image->createImage(gltfImage.width, gltfImage.height, mipLevels, VK_SAMPLE_COUNT_1_BIT, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT);

	VulkanCommand command(*device);
	command.transitionImageLayout(this->image->image(), VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mipLevels);
	VulkanCommand copyCommand(*device);
	copyCommand.copyBufferToImage(stagingBuffer->buffer(), this->image->image(), static_cast<uint32_t>(gltfImage.width), static_cast<uint32_t>(gltfImage.height));

	this->image->generateMipMaps(VK_FORMAT_R8G8B8A8_UNORM, gltfImage.width, gltfImage.height, mipLevels);
	imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
}

bool Texture::isKtx(const std::string& url)
{
	bool result = false;
	auto pos = url.find_last_of(".");
	if (pos != std::string::npos) {
		if (url.substr(pos + 1) == "ktx") {
			result = true;
		}
	}
...
Material::Material(VulkanDevice* device) : device(device) {
...
void Material::createDescriptorSet(VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout, uint32_t descriptorBindingFlags) {
	...
	auto result = vkAllocateDescriptorSets(device->logicalDevice(), &descriptorSetAllocInfo, &descriptorSet);
	if (result != VK_SUCCESS) throw std::runtime_error("failed to allocatre descriptor sets");
...
Mesh::Mesh(VulkanDevice* device, glm::mat4 matrix): device(device)
{
	uniformBlock.matrix = matrix;
	uniformBuffer = std::make_shared<VulkanBuffer>(*device, sizeof(uniformBlock), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void Node::update()
{
	if (mesh) {
		...
		if (skin) {
			...
			memcpy(mesh->uniformBuffer->data(), &mesh->uniformBlock, sizeof(mesh->uniformBlock));
		}
		else {
			memcpy(mesh->uniformBuffer->data(), &m, sizeof(glm::mat4));
		}
	}
...
{
	std::vector<VkVertexInputAttributeDescription> results;
	uint32_t location = 0;
	for (VertexComponent component : components) {
		results.push_back(Vertex::inputAttributeDescription(binding, location, component));
		location++;
	}
	return results;
}
	...
	for (tinygltf::Material& mat : gltfModel.materials) {
		...
		if (mat.additionalValues.find("alphaMode") != mat.additionalValues.end()) {
			...
			if (param.string_value == "BLEND") {
				material.alphaMode = Material::AM_BLEND;
			}
			if (param.string_value == "MASK") {
				material.alphaMode = Material::AM_MASK;
			}
		}
...
void Model::loadFromFile(std::string filename, VulkanDevice* device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	indicesCount = static_cast<uint32_t>(indexBuffer.size());
	verticesCount = static_cast<uint32_t>(vertexBuffer.size());

	assert((vertexBufferSize > 0) && (indexBufferSize > 0));


	{
		VulkanBuffer stagingBuffer(*device, vertexBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		stagingBuffer.mapMemory();
		stagingBuffer.update(vertexBuffer.data());
		verticesBuffer = std::make_shared<VulkanBuffer>(*device, vertexBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | memoryPropertyFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		verticesBuffer->copyBuffer(stagingBuffer);
	}

	{
		VulkanBuffer stagingBuffer(*device, indexBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		stagingBuffer.mapMemory();
		stagingBuffer.update(indexBuffer.data());
		verticesBuffer = std::make_shared<VulkanBuffer>(*device, vertexBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | memoryPropertyFlags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		verticesBuffer->copyBuffer(stagingBuffer);
	}
	...
	auto result = vkCreateDescriptorPool(device->logicalDevice(), &descriptorPoolCI, nullptr, &descriptorPool);
	...
	// Descriptors for per-node uniform buffers
	{
		...
		if (descriptorSetLayoutUbo == VK_NULL_HANDLE) {
			...
			result = vkCreateDescriptorSetLayout(device->logicalDevice(), &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo);
			if (result != VK_SUCCESS) throw std::runtime_error("failed to create DescriptorSet Layout");
		}
		for (auto node : nodes) {
	...
	// Descriptors for per-material images
	{
		...
		if (descriptorSetLayoutImage == VK_NULL_HANDLE) {
			...
			if (descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor) {
				...
				setLayoutBinding.binding = setLayoutBindings.size();
				setLayoutBinding.descriptorCount = 1;
...
void Model::bindBuffers(VkCommandBuffer commandBuffer)
{
	...
	const VkBuffer vertexBuffer = verticesBuffer->buffer();
	const VkBuffer indexBuffer = indicesBuffer->buffer();
	...
	if (node->mesh) {
		for (Primitive* primitive : node->mesh->primitives) {
			...
			if (renderFlags & RenderFlags::RenderOpaqueNodes) {
				skip = (material.alphaMode != Material::AM_OPAQUE);
			}
			if (renderFlags & RenderFlags::RenderAlphaMaskedNodes) {
				skip = (material.alphaMode != Material::AM_MASK);
			}
			if (renderFlags & RenderFlags::RenderAlphaBlendedNodes) {
				skip = (material.alphaMode != Material::AM_BLEND);
			}
			if (!skip) {
...
void Model::prepareNodeDescriptor(Node* node, VkDescriptorSetLayout descriptorSetLayout) {
	if (node->mesh) {
		...
		descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
		descriptorSetAllocInfo.descriptorSetCount = 1;
		auto result = vkAllocateDescriptorSets(device->logicalDevice(), &descriptorSetAllocInfo, &node->mesh->descriptorSet);
